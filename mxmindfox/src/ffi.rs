...
pub unsafe extern "C" fn mxmf_preset_load_json(json: *const c_char) -> *mut std::ffi::c_void {
    let s = match unsafe { cstr_to_str(json) } { Some(s) => s, None => return ptr::null_mut() };
...
pub unsafe extern "C" fn mxmf_preset_free(p: *mut std::ffi::c_void) {
    if p.is_null() { return; }
    unsafe { drop(Box::from_raw(p as *mut MoodPreset)); }
}
...
pub unsafe extern "C" fn mxmf_compute_mood(
    ...
    if preset.is_null() { return ptr::null_mut(); }
    let preset = unsafe { &*(preset as *const MoodPreset) };
    let cells_str = match unsafe { cstr_to_str(cells_json) } { Some(s) => s, None => return ptr::null_mut() };
...
pub unsafe extern "C" fn mxmf_compute_diplomacy_toward(
    ...
    if preset.is_null() { return 0.0; }
    let preset = unsafe { &*(preset as *const MoodPreset) };
    let cells_str = match unsafe { cstr_to_str(cells_json) } { Some(s) => s, None => return 0.0 };
    let cells: Vec<mxbs::Cell> = match serde_json::from_str(cells_str) {
        Ok(v) => v,
        Err(_) => return 0.0,
    };
...
pub unsafe extern "C" fn mxmf_adjust_threshold(
    ...
    let mood_str = match unsafe { cstr_to_str(mood_json) } { Some(s) => s, None => return base };
    let mood: Mood = match serde_json::from_str(mood_str) { Ok(v) => v, Err(_) => return base };
    let rules_str = match unsafe { cstr_to_str(rules_json) } { Some(s) => s, None => return base };
    let rules: Vec<ThresholdRule> = match serde_json::from_str(rules_str) { Ok(v) => v, Err(_) => return base };
...
pub extern "C" fn mxmf_decision_remember(
    ...
    if remember(score, threshold, temperature, &mut rng) { 1 } else { 0 }
}
...
pub unsafe extern "C" fn mxmf_decision_sample(
    ...
    struct Item { index: i32, score: f32 }

    let s = match unsafe { cstr_to_str(candidates_json) } { Some(s) => s, None => return -1 };
    let items: Vec<Item> = match serde_json::from_str(s) { Ok(v) => v, Err(_) => return -1 };
    if items.is_empty() { return -1; }
