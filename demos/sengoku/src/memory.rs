...
use mxbs::{AgentRegistry, Cell, FACTOR_DIM, MxBS, MxBSConfig};
...
pub fn event_features(event: EventType) -> [u8; 16] {
    match event {
        EventType::Conscript => [
            80, 80, 128, 128, 128, 128, 10, 20, 60, 210, 100, 80, 128, 170, 160, 80,
        ],
        EventType::AllianceFormed => [
            30, 10, 128, 128, 230, 210, 220, 20, 140, 128, 120, 150, 80, 180, 80, 60,
        ],
        EventType::AllianceRejected => [
            40, 10, 128, 128, 40, 40, 80, 20, 128, 128, 80, 100, 60, 120, 60, 40,
        ],
        EventType::BattleAttacker { won, loss_ratio } => {
            ...
            let threat = if won {
                ...
                (100.0 + loss_ratio * 155.0).min(255.0) as u8
            };
            [
                220,
                190,
        ...
        EventType::BattleDefender { won, loss_ratio } => {
            ...
            let threat = if won {
                ...
                (150.0 + loss_ratio * 105.0).min(255.0) as u8
            };
            [
                40,
                180,
        ...
        EventType::Conquered => [
            230, 220, 240, 240, 10, 20, 10, 30, 60, 50, 240, 20, 240, 250, 250, 250,
        ],
        EventType::Eliminated => [
            20, 20, 20, 20, 10, 10, 10, 10, 20, 10, 250, 250, 10, 10, 10, 250,
        ],
    }
}
...
fn store_event_with_features(
    ...
    if mode == 0o744 {
        reg.store_public(mxbs, turn, owner_slug, text, features, price)
            .unwrap()
    } else {
        reg.store_private(mxbs, turn, owner_slug, text, features, price)
            .unwrap()
    }
}
...
pub fn get_memories_for_agent(
    ...
    let results = reg
        .search(mxbs, slug, query_features, current_turn)
        .unwrap();
    results.iter().map(|r| r.text.clone()).collect()
}

pub async fn score_all_pending(mxbs: &MxBS, preset_json: &str) {
    ...
    let texts: Vec<(u64, String)> = unscored.iter().map(|c| (c.id, c.text.clone())).collect();

    let batch_count = texts.len().div_ceil(4);
    println!(
        "  スコアリング中（{}件, {}バッチ）",
        texts.len(),
        batch_count
    ...
    println!(
        "  完了（{}件, 合計{:.1}秒, 平均{:.2}秒/件）",
        texts.len(),
        total.as_secs_f64(),
        total.as_secs_f64() / texts.len() as f64
    );
}
...
pub fn compute_mood(recent_features: &[[u8; 16]]) -> crate::types::Mood {
    ...
    let avg_inv = |indices: &[usize]| -> f32 {
        let sum: f32 = recent_features
            .iter()
            .map(|f| {
                indices.iter().map(|&i| (255 - f[i]) as f32).sum::<f32>() / indices.len() as f32
            })
            .sum();
        (sum / n) / 255.0
    };
...
pub fn get_agent_mood(
    ...
    let results = mxbs
        .search(query, owner_id, viewer_bit)
    ...
    let features: Vec<[u8; 16]> = results
        .iter()
...
mod tests {
    ...
    fn test_event_features_battle_attacker_won() {
        let f = event_features(EventType::BattleAttacker {
            won: true,
            loss_ratio: 0.2,
        });
        assert!(f[2] > 150);
    ...
    fn test_event_features_battle_defender_lost() {
        let f = event_features(EventType::BattleDefender {
            won: false,
            loss_ratio: 0.8,
        });
        assert!(f[2] < 80);
    ...
    fn test_mood_aggressive() {
        let features = [[
            200, 128, 200, 230, 128, 128, 128, 128, 128, 128, 128, 128, 128, 200, 220, 128,
        ]];
        let mood = compute_mood(&features);
        assert!(
            mood.aggression > 0.7,
            "aggression should be high, got {}",
            mood.aggression
        );
    }
    ...
    fn test_mood_desperate() {
        let features = [[
            128, 128, 128, 50, 128, 128, 128, 128, 50, 128, 220, 230, 128, 30, 128, 200,
        ]];
        let mood = compute_mood(&features);
        assert!(
            mood.desperation > 0.6,
            "desperation should be high, got {}",
            mood.desperation
        );
    }
    ...
    fn test_mood_low_trust_rejects() {
        let features = [[
            128, 128, 128, 128, 30, 230, 30, 128, 128, 128, 128, 128, 128, 128, 128, 128,
        ]];
        let mood = compute_mood(&features);
        assert!(
            mood.diplomacy < 0.3,
            "diplomacy should be low, got {}",
            mood.diplomacy
        );
    }
}
