...
```
=== Campaign Summary (50 games, 1,420 turns total, elapsed 98s) ===

🎯 ページワン忘却率:
  キャラ    | price | 宣言機会 | 忘れた回数 | 忘却率  | 指摘された | 逃げた
  ----------+-------+----------+-----------+---------+-----------+-------
  エルマー  |  170  |    82    |     9     |  11.0%  |     6     |   3
  ノクちん  |   70  |    64    |    51     |  79.7%  |    31     |  20
  スミレ    |  220  |    78    |     1     |   1.3%  |     1     |   0
  ティル    |   80  |    59    |    46     |  78.0%  |    25     |  21
  ヴェリ    |  200  |    71    |     3     |   4.2%  |     2     |   1

🔔 指摘成功率（指摘者として）:
  キャラ    | 指摘機会 | 成功 | 失敗（自分も忘れてた）
  ----------+----------+------+---------------------
  エルマー  |    28    |  23  |   5
  ノクちん  |    22    |   4  |  18
  スミレ    |    31    |  31  |   0
...
📉 price vs 忘却ターン（初回忘却までのターン数）:
  price  70 (ノクちん): 平均 7.4 ターン (σ=2.1)
  price  80 (ティル):   平均 9.1 ターン (σ=2.5)
  price 170 (エルマー): 平均 21.6 ターン (σ=4.3, reinforce効果込み)
  price 200 (ヴェリ):   平均 28.2 ターン (σ=3.8)
  price 220 (スミレ):   平均 31.5 ターン (σ=5.1)
```
...
おやつデモ（gemma4:26b）の 1 ゲーム 20〜30 分と比較して **約 1000 倍高速**。
...
## 11. preset_guide.md への貢献

本デモの実測データから、以下をガイドに記載する：

1. **price 設計指針**: 「price=70 なら half_life=8 で約 N ターンで検索圏外」の実測テーブル（50ゲーム平均 + 標準偏差）
2. **reinforce 効果**: 「目撃 reinforce +0.4 で寿命が約 M ターン延長」の実測値
3. **忘却カーブグラフ**: price 別の effective_score 推移（50ゲーム平均）
4. **half_life 比較**: half_life=4/8/16 での忘却速度比較テーブル
5. **閾値設計**: 「THRESHOLD=0.3 でゲーム体験として自然か」の所感
